...
## Crash dump: analyze a HardFault in the field

```bash
# Board is stuck — capture full crash context
vsf-bench-debug crash-dump --board b1 --project application-standalone board/
```

JSON output includes PC, SP, LR, fault classification (e.g. `"HARD_FAULT"`),
stack backtrace with function names and source locations, and assertion context
if the fault was a `vsf_trace_assert`.

## Backtrace: inspect current execution

```bash
# Halt CPU, walk the call stack
vsf-bench-debug backtrace --board b1 --project application-standalone board/
```

Useful after `break` hits or when the board appears to be in a stuck loop.
Frames include PC, SP, LR, resolved function name, file, and line number.

## Breakpoint workflow: halt at function entry

```bash
# Set BP at main, resume, wait for hit
...
vsf-bench-debug vars --board b1 --project application-standalone --name g_init_state board/
vsf-bench-debug backtrace --board b1 --project application-standalone board/
...
## Variable inspection: check global state

```bash
# Single variable
vsf-bench-debug vars --board b1 --project application-standalone --name vsf_bt_state board/

# All VSF trace buffers
vsf-bench-debug vars --board b1 --project application-standalone --name "vsf_trace_*" board/

# All globals matching a pattern
vsf-bench-debug vars --board b1 --project application-standalone --name "*state*" board/
```

Output includes raw bytes, uint8/16/32/64 interpretations, string rendering,
and pointer dereferencing.

## Memory dump: inspect raw memory region

```bash
vsf-bench-debug read --board b1 --addr 0x20000000 --len 512 board/
```
