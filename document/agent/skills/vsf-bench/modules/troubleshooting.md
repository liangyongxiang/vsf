...
| Symptom | Likely cause | Action |
|---------|-------------|--------|
| "board has no debug_probe" | `debug_probe` not configured for this board | add `debug_probe: {target: cortex_m}` to board entry in hardware-map.yml |
| pyOCD import error | pyocd not installed | `pip install pyocd` |
| pyelftools import error | pyelftools not installed | `pip install pyelftools` |
| Probe not found | no CMSIS-DAP/J-Link connected or driver issue | check USB connection; verify probe appears in Device Manager; try different USB cable |
| Multiple probes, wrong one selected | pyOCD picks first available probe | set `debug_probe.probe` to target probe's unique ID (serial number) |
| Connection timeout | target in low-power/debug-disabled state | power-cycle board; check firmware doesn't disable SWD pins |
| ELF not found | no `--elf` or `--project` given, or artifact format mismatch | use explicit `--elf <path>` or verify project `build.artifacts` has `format: elf` or `format: out` |
| Symbol not found | function/variable name misspelled or not in `.symtab` | verify with `arm-none-eabi-nm build/app.out \| grep <name>` |
| Breakpoint not hit | wrong address, code path not reached, or breakpoint overrun | verify address against disassembly; check code is reachable; try setting BP earlier in call chain |
| "halt" returns error | target already running or in reset | power-cycle board; try again |
| Data read returns zeros | wrong address or memory not accessible | verify address in linker map; check memory region not powered down |
| Variable shows 0 | variable optimized away or wrong ELF | check build with debug symbols (`-g`); verify variable not in register/lto-eliminated |
