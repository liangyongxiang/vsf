...
**Unimplemented APIs:** every stub must `VSF_HAL_ASSERT(0);` AND return an error (`VSF_ERR_NOT_SUPPORT` for "hardware can't") or a sentinel (`0`/`-1`) — never a silent `return VSF_ERR_NONE;` after only a pointer assert. See REFERENCE.md "Unimplemented API convention".
