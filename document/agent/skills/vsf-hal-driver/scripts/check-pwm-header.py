#!/usr/bin/env python3
"""Deterministic check: PWM header completeness.
Usage: check-pwm-header.py <pwm.h>
...
def check_header(path: str) -> tuple[int, int]:
    ...
    # ── Template include ──
    if has(r'#include.*vsf_template_pwm\.h'):
        say("OK", "vsf_template_pwm.h included")
    elif is_ipcore:
        say("OK", "types provided by IPCore header include")
    else:
        say("WARN", "missing #include vsf_template_pwm.h (OK if types from driver.h)")

    # ── irq_mask_t ──
    if has(r"vsf_pwm_irq_mask_t"):
        say("OK", "vsf_pwm_irq_mask_t defined")
    elif is_ipcore:
        pass
    else:
        say("WARN", "vsf_pwm_irq_mask_t not found (OK if from driver.h)")

    # ── IRQ bits ──
    if not is_ipcore:
        for bit in ("CPL",):
            macro = f"VSF_PWM_IRQ_MASK_{bit}"
    ...
    # ── vsf_pwm_isr_t ──
    if has(r"vsf_pwm_isr_t"):
        say("OK", "vsf_pwm_isr_t defined")
    elif not is_ipcore:
        say("WARN", "vsf_pwm_isr_t not found")
...
def main():
    if len(sys.argv) < 2:
        print(f"Usage: {sys.argv[0]} <pwm.h>")
        sys.exit(1)
    path = sys.argv[1]
