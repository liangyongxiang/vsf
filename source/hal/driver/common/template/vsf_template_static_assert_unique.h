 ...
 *  You may obtain __A copy of the License at                                  *
 *                                                                           *
 ...
 * #define CUSTOM_CHECK(__A, __B) VSF_STATIC_ASSERT((__A) != (__B) && (__A) > 0
 * && (__B) > 0) VSF_HAL_CHECK_UNIQUE_CUSTOM(CUSTOM_CHECK, MODE_A, MODE_B,
 * MODE_C);
 * @endcode
 *
 * Check modes:
...
#define VSF_HAL_CHECK_MODE_LOOSE(__A, __B)                                     \
  static_assert((__A) != (__B) || ((__A) == 0 && (__B) == 0),                 \
                "Values must be different or both zero")
#define VSF_HAL_CHECK_MODE_STRICT(__A, __B)                                    \
  static_assert(((__A) & (__B)) == 0,                                          \
                "Values must have disjoint bits")

/* Check mode for all values being zero */
#define VSF_HAL_CHECK_MODE_ALL_ZERO(...)                                       \
  static_assert(__VSF_HAL_ZERO_COUNT_ALL_VALUES(__VA_ARGS__) ==                \
...
#define VSF_HAL_CHECK_MODE_ALL_NON_ZERO(...)                                   \
  static_assert(__VSF_HAL_ZERO_COUNT_ALL_VALUES(__VA_ARGS__) == 0,            \
                "All values must be non-zero")

/* Check mode for specific number of zeros */
#define VSF_HAL_CHECK_MODE_ZERO_COUNT(__COUNT, ...)                            \
  static_assert(__VSF_HAL_ZERO_COUNT_ALL_VALUES(__VA_ARGS__) == (__COUNT),    \
                "Incorrect number of zero values")
...
#define ____VSF_HAL_ZERO_COUNT_BASE(__A, __B) (((__A) == 0) + (__B))
