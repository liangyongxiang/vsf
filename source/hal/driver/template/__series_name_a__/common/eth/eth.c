...
vsf_err_t VSF_MCONNECT(VSF_ETH_CFG_IMP_PREFIX, _eth_sg_send_request)(
    VSF_MCONNECT(VSF_ETH_CFG_IMP_PREFIX, _eth_t) *eth_ptr,
    vsf_eth_sg_send_buf_t *buf_ptr,
    uint32_t sg_count
) {
    VSF_HAL_ASSERT((NULL != eth_ptr) && (NULL != buf_ptr));
...
vsf_err_t VSF_MCONNECT(VSF_ETH_CFG_IMP_PREFIX, _eth_sg_recv_request)(
    VSF_MCONNECT(VSF_ETH_CFG_IMP_PREFIX, _eth_t) *eth_ptr,
    vsf_eth_sg_recv_buf_t *buf_ptr,
    uint32_t sg_count
) {
    VSF_HAL_ASSERT((NULL != eth_ptr) && (NULL != buf_ptr));
...
static void VSF_MCONNECT(__, VSF_ETH_CFG_IMP_PREFIX, _eth_irqhandler)(
    ...
    if (isr_ptr->handler_fn != NULL) {
        isr_ptr->handler_fn(isr_ptr->target_ptr, (vsf_eth_t *)eth_ptr);
    }
}
// HW end
