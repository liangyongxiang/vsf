...
def _entry_to_board(entry: dict, yaml_dir: Path, hubs: dict | None = None, gpio_adapters: dict | None = None) -> BoardConfig:
    ...
    # ── Serial ports ──
    serial_ports: dict[str, SerialPortConfig] = {}
    sp_raw = entry.get("serial_ports", {})
    if sp_raw:
        for name, sp_entry in sp_raw.items():
            match = sp_entry.get("match")
            port = ""
            if isinstance(match, dict):
                port = _match_serial_port(match, label=f"board {_derive_board_name(entry)} {name} UART")
            sp = SerialPortConfig(
                port=port or sp_entry.get("port", ""),
                baudrate=sp_entry.get("baudrate", 115200),
                match=match,
            )
            serial_ports[name] = sp
    else:
        # Old format not supported — give clear error
        if "debug_uart" in entry or "program_uart" in entry or "serial" in entry:
            raise ValueError(
                f"Board '{_derive_board_name(entry)}' uses old serial format "
                "(debug_uart/program_uart/serial). Migrate to serial_ports: "
                "{debug: {match: {vid, pid, serial, desc}, baudrate: ...}, "
                "program: {...}}"
            )
