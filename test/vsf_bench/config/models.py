...
class StepType(Enum):
    ...
    VENDOR_LOG_START = "vendor_log_start"
    VENDOR_LOG_STOP = "vendor_log_stop"
...
class SerialPortConfig:
    """Serial port descriptor — port, baud rate, and optional match criteria."""
    port: str = ""                # resolved COM port
    baudrate: int = 115200
    match: dict | None = None     # {vid, pid, serial, desc} — raw match criteria


@dataclass
class BoardConfig:
    platform: str
    serial_ports: dict[str, SerialPortConfig] = field(default_factory=dict)
    connected: bool = True
