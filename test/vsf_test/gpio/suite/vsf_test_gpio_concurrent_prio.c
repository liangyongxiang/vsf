...
void vsf_test_gpio_concurrent_prio_run(const vsf_test_suite_t *suite, const vsf_test_case_t *tc, const void *fixture)
{
    ...
    vsf_trace_info("GPIO:CONCURRENT:cb=%lu main=%lu" VSF_TRACE_CFG_LINEEND,
                   (unsigned long)vsf_test_suite_data.gpio.gpio_concurrent_prio.callback_toggles,
                   (unsigned long)vsf_test_suite_data.gpio.gpio_concurrent_prio.main_toggles);
    /* Both streams must have run. */
    VSF_TEST_ASSERT(vsf_test_suite_data.gpio.gpio_concurrent_prio.callback_toggles > 0);
    VSF_TEST_ASSERT(vsf_test_suite_data.gpio.gpio_concurrent_prio.main_toggles > 0);
}
