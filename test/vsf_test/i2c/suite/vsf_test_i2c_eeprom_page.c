...
void vsf_test_i2c_eeprom_page_run(const vsf_test_suite_t *suite, const vsf_test_case_t *tc, const void *fixture)
{
    ...
    for (uint8_t i = 0; i < data_len; i++) {
        vsf_test_suite_data.i2c.i2c_eeprom_page.write_buf[1 + i] = (uint8_t)(0xB0 + i);
    }
    ...
    for (uint8_t i = 0; i < data_len; i++) {
        VSF_TEST_ASSERT(vsf_test_suite_data.i2c.i2c_eeprom_page.read_buf[i] == vsf_test_suite_data.i2c.i2c_eeprom_page.write_buf[1 + i]);
    }
