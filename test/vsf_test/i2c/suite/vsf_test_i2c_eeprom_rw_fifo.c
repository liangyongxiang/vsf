                      ...
                      vsf_i2c_irq_mask_t irq_mask)
{
    ...
    vsf_test_i2c_eeprom_rw_fifo_suite_t *suite =
        (vsf_test_i2c_eeprom_rw_fifo_suite_t *)target_ptr;
    suite->irq_mask |= irq_mask;
}

static bool __wait_irq(vsf_test_i2c_eeprom_rw_fifo_suite_t *suite,
                       vsf_i2c_irq_mask_t check_mask, uint32_t timeout_ms)
{
    while (timeout_ms-- > 0) {
        if (suite->irq_mask & check_mask) return true;
...
static bool __eeprom_ack_poll(vsf_test_i2c_eeprom_rw_fifo_suite_t *suite,
...
void vsf_test_i2c_eeprom_rw_fifo_run(const vsf_test_i2c_eeprom_rw_fifo_case_t *c)
{
    ...
    VSF_TEST_ASSERT(__fifo_write(i2c, c->eeprom_addr,
        VSF_I2C_CMD_START | VSF_I2C_CMD_STOP | VSF_I2C_CMD_WRITE | VSF_I2C_CMD_7_BITS,
        data_len + 1, c->suite->write_buf, VSF_TEST_I2C_FIFO_TIMEOUT_MS));
    ...
    VSF_TEST_ASSERT(err == VSF_ERR_NONE);
    VSF_TEST_ASSERT(__wait_irq(c->suite,
        VSF_I2C_IRQ_MASK_MASTER_TRANSFER_COMPLETE | VSF_I2C_IRQ_MASK_MASTER_ERR, 1000));
    VSF_TEST_ASSERT(c->suite->irq_mask & VSF_I2C_IRQ_MASK_MASTER_TRANSFER_COMPLETE);
