                         ...
                         vsf_i2c_irq_mask_t irq_mask)
{
    ...
    vsf_test_suites.i2c_slave_fifo.master_irq_mask |= irq_mask;
    if (irq_mask & VSF_I2C_IRQ_MASK_MASTER_TRANSFER_COMPLETE) {
        vsf_test_suites.i2c_slave_fifo.master_done = true;
    }
}
...
{
    vsf_test_suite_t *suite = target_ptr;
    vsf_test_suites.i2c_slave_fifo.slave_irq_mask |= irq_mask;
    ...
    if (irq_mask & (VSF_I2C_IRQ_MASK_SLAVE_TRANSFER_COMPLETE | VSF_I2C_IRQ_MASK_SLAVE_STOP_DETECT)) {
        vsf_test_suites.i2c_slave_fifo.slave_complete = true;
    }
}
...
{
    while (timeout_ms-- > 0) {
        if (vsf_test_suites.i2c_slave_fifo.master_done) return true;
...
{
    while (timeout_ms-- > 0) {
        if (vsf_test_suites.i2c_slave_fifo.slave_complete) return true;
        vsf_test_busy_wait_ms(1);
    }
    return false;
}
...
void vsf_test_i2c_slave_fifo_run(const vsf_test_suite_t *suite, const vsf_test_case_t *tc, const void *fixture)
{
    ...
    vsf_test_suites.i2c_slave_fifo.master_irq_mask = 0;
    vsf_test_suites.i2c_slave_fifo.slave_irq_mask  = 0;
    memset(vsf_test_suites.i2c_slave_fifo.master_buf, 0, sizeof(vsf_test_suites.i2c_slave_fifo.master_buf));
    memset(vsf_test_suites.i2c_slave_fifo.slave_buf, 0, sizeof(vsf_test_suites.i2c_slave_fifo.slave_buf));
    vsf_test_suites.i2c_slave_fifo.slave_rx_offset = 0;
    vsf_test_suites.i2c_slave_fifo.master_done     = false;
    vsf_test_suites.i2c_slave_fifo.slave_complete  = false;
    ...
    err = vsf_i2c_master_request(master_i2c, VSF_TEST_I2C_SLAVE_FIFO_ADDR,
        ...
        16, vsf_test_suites.i2c_slave_fifo.master_buf);
    VSF_TEST_ASSERT(err == VSF_ERR_NONE);
