...
{
    (void)timer_ptr;
    vsf_test_timer_async_suite_t *suite = (vsf_test_timer_async_suite_t *)target_ptr;
    if (irq_mask & VSF_TIMER_IRQ_MASK_OVERFLOW) {
        if (suite->counter < 255) {
            suite->counter++;
        }
    }
}
...
{
    vsf_test_timer_async_case_t *c = (vsf_test_timer_async_case_t *)arg;
    ...
    while (c->suite->counter < 1 && timeout_ms-- > 0) {
        vsf_test_busy_wait_ms(1);
    }
    VSF_TEST_ASSERT(c->suite->counter == 1);
    ...
    timeout_ms = 200;
    while (c->suite->counter < TIMER_ASYNC_COUNT && timeout_ms-- > 0) {
        vsf_test_busy_wait_ms(1);
    }
    VSF_TEST_ASSERT(c->suite->counter == TIMER_ASYNC_COUNT);
    ...
    timeout_ms = 50;
    while (c->suite->counter < 3 && timeout_ms-- > 0) {
        vsf_test_busy_wait_ms(1);
    }
    VSF_TEST_ASSERT(c->suite->counter >= 3);

    err = vsf_timer_channel_request_stop(timer, 0);
    VSF_TEST_ASSERT(err == VSF_ERR_NONE);

    uint8_t count_after_stop = c->suite->counter;
    vsf_test_busy_wait_ms(50);
    VSF_TEST_ASSERT(c->suite->counter == count_after_stop);
