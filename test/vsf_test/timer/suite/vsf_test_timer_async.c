                        ...
                        vsf_timer_irq_mask_t irq_mask)
{
    ...
    if (irq_mask & VSF_TIMER_IRQ_MASK_OVERFLOW) {
        if (vsf_test_suite_data.timer_async.counter < 255) {
            vsf_test_suite_data.timer_async.counter++;
        }
    }
}
...
void vsf_test_timer_async_run(const vsf_test_suite_t *suite, const vsf_test_case_t *tc, const void *fixture)
{
    ...
    vsf_test_suite_data.timer_async.counter = 0;
    ...
    while (vsf_test_suite_data.timer_async.counter < 1 && timeout_ms-- > 0) {
        vsf_test_busy_wait_ms(1);
    }
    VSF_TEST_ASSERT(vsf_test_suite_data.timer_async.counter == 1);
    ...
    timeout_ms = 200;
    while (vsf_test_suite_data.timer_async.counter < TIMER_ASYNC_COUNT && timeout_ms-- > 0) {
        vsf_test_busy_wait_ms(1);
    }
    VSF_TEST_ASSERT(vsf_test_suite_data.timer_async.counter == TIMER_ASYNC_COUNT);
    ...
    timeout_ms = 50;
    while (vsf_test_suite_data.timer_async.counter < 3 && timeout_ms-- > 0) {
        vsf_test_busy_wait_ms(1);
    }
    VSF_TEST_ASSERT(vsf_test_suite_data.timer_async.counter >= 3);

    err = vsf_timer_channel_request_stop(timer, 0);
    VSF_TEST_ASSERT(err == VSF_ERR_NONE);

    uint8_t count_after_stop = vsf_test_suite_data.timer_async.counter;
    vsf_test_busy_wait_ms(50);
    VSF_TEST_ASSERT(vsf_test_suite_data.timer_async.counter == count_after_stop);
