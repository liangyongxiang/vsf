                        ...
                        vsf_timer_irq_mask_t irq_mask)
{
    ...
    if (irq_mask & VSF_TIMER_IRQ_MASK_OVERFLOW) {
        if (vsf_test_suite_data.timer.timer_async.counter < 255) {
...
void vsf_test_timer_async_run(const vsf_test_suite_t *suite, const vsf_test_case_t *tc, const void *fixture)
{
    ...
    while (vsf_test_suite_data.timer.timer_async.counter < 1 && timeout_ms-- > 0) {
    ...
    timeout_ms = 200;
    while (vsf_test_suite_data.timer.timer_async.counter < TIMER_ASYNC_COUNT && timeout_ms-- > 0) {
        vsf_test_busy_wait_ms(1);
    }
    VSF_TEST_ASSERT(vsf_test_suite_data.timer.timer_async.counter == TIMER_ASYNC_COUNT);
    ...
    timeout_ms = 50;
    while (vsf_test_suite_data.timer.timer_async.counter < 3 && timeout_ms-- > 0) {
        vsf_test_busy_wait_ms(1);
    }
