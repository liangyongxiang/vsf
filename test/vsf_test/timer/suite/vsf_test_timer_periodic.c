                        ...
                        vsf_timer_irq_mask_t irq_mask)
{
    ...
    if (irq_mask & VSF_TIMER_IRQ_MASK_OVERFLOW) {
        if (vsf_test_suite_data.timer.timer_periodic.counter < TIMER_PERIODIC_COUNT) {
...
void vsf_test_timer_periodic_run(const vsf_test_suite_t *suite, const vsf_test_case_t *tc, const void *fixture)
{
    ...
    while (vsf_test_suite_data.timer.timer_periodic.counter < TIMER_PERIODIC_COUNT && timeout_ms-- > 0) {
        vsf_test_busy_wait_ms(1);
    }

    VSF_TEST_ASSERT(vsf_test_suite_data.timer.timer_periodic.counter == TIMER_PERIODIC_COUNT);
