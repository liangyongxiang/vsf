...
#ifndef RX_BAUD_PAYLOAD
#   define RX_BAUD_PAYLOAD          "Hello VSF\r\n"
#endif
#ifndef MARKER_DELAY_MS
#   define MARKER_DELAY_MS          200
#endif
#ifndef RX_BAUD_PAYLOAD_DRAIN_MS
#   define RX_BAUD_PAYLOAD_DRAIN_MS 500
#endif
#ifndef RX_BAUD_COMMON_MODE
#   define RX_BAUD_COMMON_MODE      (VSF_USART_NO_PARITY | VSF_USART_1_STOPBIT | VSF_USART_8_BIT_LENGTH | VSF_USART_RX_ENABLE)
#endif
...
void vsf_test_usart_rx_baud_scenario(void *arg)
{
    const vsf_test_usart_rx_baud_case_t *c = (const vsf_test_usart_rx_baud_case_t *)arg;
    ...
    if (c->expect_pass) {
        ...
        uint8_t rx_buf[32];
        uint16_t rx_len = 0;
        const char *expected = RX_BAUD_PAYLOAD;
        uint16_t expected_len = strlen(expected);

        uint32_t timeout_ticks = vsf_systimer_get_ms() + RX_BAUD_PAYLOAD_DRAIN_MS * 10;
