...
static void __tx_fifo_isr(void *target, vsf_usart_t *usart, vsf_usart_irq_mask_t irq_mask)
{
    ...
    vsf_test_usart_tx_fifo_irq_suite_t *suite = (vsf_test_usart_tx_fifo_irq_suite_t *)target;
    suite->isr_count++;
     ...
     * returns only 0/1. */
    while (suite->remaining > 0) {
        uint_fast16_t want = (suite->remaining > 64) ? 64 : (uint_fast16_t)suite->remaining;
        uint_fast16_t wrote = vsf_usart_txfifo_write(usart, (void *)suite->src, want);
        suite->src       += wrote;
        suite->remaining -= wrote;
...
void vsf_test_usart_tx_fifo_irq_run(const vsf_test_usart_tx_fifo_irq_case_t *c)
{
    ...
    vsf_err_t err = vsf_usart_init(usart, &(vsf_usart_cfg_t){
        ...
        .baudrate = 115200,
        .isr      = { .handler_fn = __tx_fifo_isr, .target_ptr = c->suite,
    ...
    if (prefill_request > c->suite->remaining) {
        prefill_request = (uint_fast16_t)c->suite->remaining;
    }
    uint_fast16_t wrote = vsf_usart_txfifo_write(usart, (void *)c->suite->src, prefill_request);
    c->suite->src       += wrote;
    ...
    vsf_trace_info("USART:TX_FIFO_IRQ:isr=%lu total=%lu" VSF_TRACE_CFG_LINEEND,
                   (unsigned long)c->suite->isr_count, (unsigned long)total);
