...
void vsf_test_usart_break_signal_run(const vsf_test_suite_t *suite, const vsf_test_case_t *tc, const void *fixture)
{
    ...
    VSF_TEST_ASSERT(vsf_usart_ctrl((vsf_usart_t *)fixture, VSF_USART_CTRL_SET_BREAK, NULL) == VSF_ERR_NONE);
    vsf_test_busy_wait_ms(p->hold_ms);
    VSF_TEST_ASSERT(vsf_usart_ctrl((vsf_usart_t *)fixture, VSF_USART_CTRL_CLEAR_BREAK, NULL) == VSF_ERR_NONE);
