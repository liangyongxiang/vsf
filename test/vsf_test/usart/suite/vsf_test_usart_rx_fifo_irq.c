...
void vsf_test_usart_rx_fifo_irq_run(const vsf_test_suite_t *suite, const vsf_test_case_t *tc, const void *fixture)
{
    ...
    for (uint32_t iter = 0; iter < RX_FIFO_IRQ_POLL_MAX_ITER && !vsf_test_suite_data.usart.usart_rx_fifo_irq.done; iter++) {
    ...
    vsf_trace_info("USART:RX_FIFO_IRQ:isr=%lu got=%lu" VSF_TRACE_CFG_LINEEND,
                   (unsigned long)vsf_test_suite_data.usart.usart_rx_fifo_irq.isr_count, (unsigned long)vsf_test_suite_data.usart.usart_rx_fifo_irq.received);
