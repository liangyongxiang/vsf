...
void vsf_test_usart_rx_irq_run(vsf_test_case_t *tc)
{
    ...
    if (p->expect_pass) {
        ...
        {
            uint8_t junk[16];
            while (vsf_usart_rxfifo_get_data_count((vsf_usart_t *)suite->arg) > 0) {
                if (vsf_usart_rxfifo_read((vsf_usart_t *)suite->arg, junk, sizeof(junk)) == 0) break;
            }
        }

        vsf_usart_irq_enable((vsf_usart_t *)suite->arg, VSF_USART_IRQ_MASK_RX | VSF_USART_IRQ_MASK_RX_TIMEOUT);
        ...
        vsf_usart_irq_disable((vsf_usart_t *)suite->arg, VSF_USART_IRQ_MASK_RX | VSF_USART_IRQ_MASK_RX_TIMEOUT);
